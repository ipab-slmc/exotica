...
EReturn IKsolver::initDerived(tinyxml2::XMLHandle & handle) {
	try {
		...
		if (frrthandle.ToElement()) {
			...
			server_->registerParam<std_msgs::Bool>(ns_ + "/FRRTLocal",
					tmp_handle, ignore_obs_near_goal_);
...
EReturn IKsolver::specifyProblem(PlanningProblem_ptr pointer) {
	...
	for (auto &it : tasks_) {
		...
		} else if (it.second->getTaskMap()->type().compare(
				"exotica::Orientation") == 0) {
			reach_orientation_taskmap_ =
					boost::static_pointer_cast<Orientation>(
							it.second->getTaskMap());
		}
	}
	initialised_ = true;
...
EReturn IKsolver::setGoal(const std::string & task_name,
	...
	if (taskIndex.find(task_name) == taskIndex.end()) {
		std::cout << "Task name " << task_name << " does not exist"
				<< std::endl;
		return FAILURE;
	} else {
...
EReturn IKsolver::Solve(Eigen::VectorXdRefConst q0,
	...
	if (initialised_) {
		...
		if (size_ != q0.rows()) {
			std::cout << "Wrong size q0 size=" << q0.rows()
					<< ", required size=" << size_ << std::endl;
			INDICATE_FAILURE
			return FAILURE;
		}
		...
		if (!use_drm_->data) {
			...
			if (reach_position_taskmap_) {
				reach_goal_.position.x =
						reach_position_taskmap_->ref_pose_.p[0];
				reach_goal_.position.y =
						reach_position_taskmap_->ref_pose_.p[1];
				reach_goal_.position.z =
						reach_position_taskmap_->ref_pose_.p[2];
			} else {
			...
			for (int j = 0; j < 10; j++) {
				...
				if (drm_client_.sendGoalAndWait(goal, ros::Duration(2)).state_
					...
					if (drm_client_.getResult()->succeed) {
						if (ok(
								vectorExoticaToEigen(
										drm_client_.getResult()->q_out, q))) {
							for (int t = 0; t < T; t++) {
								if (ok(
										Solve(q, solution.block(t, 0, 1, size_),
												t))) {
									q = solution.row(t);
								} else {
									INDICATE_FAILURE
...
EReturn IKsolver::Solve(Eigen::VectorXdRefConst q0, Eigen::MatrixXdRef solution,
		int t) {
	if (initialised_) {
		...
		for (int i = 0; i < maxit_->data; i++) {
			if (ok(prob_->update(solution.row(0), t))) {
				...
				if (error <= prob_->getTau() * 2.0) {
					...
					solution.row(0) = solution.row(0)
							+ vel_vec_.transpose() * 0.5;
				}
		...
		if (found) {
			...
			WARNING(
					"Solution not found after reaching max number of iterations");
			return WARNING;
		}
	} else {
		INDICATE_FAILURE
...
EReturn IKsolver::vel_solve(double & err, int t, Eigen::VectorXdRefConst q) {
	...
	if (initialised_) {
		...
		if (FRRT_) {
			...
			if (cspace_err.squaredNorm()
					< 0.05 * init_cspace_err.squaredNorm()) {
				Eigen::VectorXd cancel = Eigen::VectorXd::Zero(
						task_error.rows());
