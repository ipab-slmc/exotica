#include "testing_pkg/Kinematica/PrismaticTest.h"

TEST_F(KinematicaYoubotTest, DefaultInitialisation)
{
...
TEST_F(KinematicaYoubotTest, InvalidJoints)
{
  robot_solution_.zero_other_joints = false;
  robot_solution_.joints_update.pop_back();  //!< Reduce the size to invalidate number
  EXPECT_FALSE(robot_tree_.initKinematics(robot_urdf_, robot_solution_)) << "Ignores incorrect sizes of joints even if zero_other_joints is false";
  robot_solution_.zero_other_joints = true;
  EXPECT_TRUE(robot_tree_.initKinematics(robot_urdf_, robot_solution_)) << "Does not initialise without correct number of joints even with zeroing flag being true";
  robot_solution_.joints_update = {"blabla"};
  EXPECT_FALSE(robot_tree_.initKinematics(robot_urdf_, robot_solution_)) << "Does not recognise invalid joints";
}

TEST_F(KinematicaYoubotTest, UpdatingKinematics)
{
  //!< Temporaries
  ...
  EXPECT_FALSE(robot_tree_.updateConfiguration(Eigen::VectorXd::Zero(NJOINTS))) << "Does not recognise unitialisation";
  ...
  EXPECT_FALSE(robot_tree_.updateConfiguration(Eigen::VectorXd::Zero(NJOINTS-1))) << "Does not fail when Incorrect number of joints specified";
  ...
  EXPECT_EQ(KDL::Vector(0.5, 0.4, 0.2), relative_pose.p - global_pose.p) << "Did Not transform FK : " << global_pose.p.x() << "/" << relative_pose.p.x() << " " << global_pose.p.y() << "/" << relative_pose.p.y() << " " << global_pose.p.z() << "/" << relative_pose.p.z();
  
  robot_solution_.root_segment = {SEGMENT_2};  //!< Use another base
  robot_solution_.ignore_unused_segs = true;   //!< so now will not update
  ASSERT_TRUE(robot_tree_.initKinematics(robot_urdf_, robot_solution_)) << "Cannot proceed with Test since could Not Initialise Robot";
  ASSERT_TRUE(robot_tree_.updateConfiguration(joint_config_1_)) << "Could not update kinematics";
  ASSERT_TRUE(robot_tree_.getPose(SEGMENT_1, incorrect_pose));
  EXPECT_NE(global_pose, incorrect_pose) << "ignore_unused_segs flag is having no effect";
}

TEST_F(KinematicaYoubotTest, CheckingKinematicsInternally)
{
  //!< Temporaries & Initialisation
  KDL::Frame pose_1, pose_2, pose_3, pose_4;
  double x_1, y_1, z_1, w_1, x_2, y_2, z_2, w_2, x_3, y_3, z_3, w_3, x_4, y_4, z_4, w_4;
  ASSERT_TRUE(robot_tree_.initKinematics(robot_urdf_, robot_solution_)) << "Cannot proceed with Test since could Not Initialise Robot";
  ASSERT_TRUE(robot_tree_.updateConfiguration(joint_config_1_)) << "Could not update kinematics";
  ...
  EXPECT_TRUE(KDL::Equal(pose_3, pose_4, 0.001)) << "Inversion not correct: x(" << std::setprecision(4) << pose_3.p.x() << "/" << pose_4.p.x() << ") : y(" << pose_3.p.y() << "/" <<  pose_4.p.y() << ") : z(" << pose_3.p.z() << "/" << pose_4.p.z() << ") : q_x(" << x_3 << "/" << x_4 << ") : q_y(" << y_3 << "/" << y_4 << ") : q_z(" << z_3 << "/" << z_4 << ") : q_w(" << w_3 << "/" << w_4 << ")";  
  ...
  pose_3.M.GetQuaternion(x_3, y_3, z_3, w_3);
  pose_4.M.GetQuaternion(x_4, y_4, z_4, w_4);
  EXPECT_TRUE(KDL::Equal(pose_3, pose_4, 0.0001)) << "Triangulation not correct: x(" << std::setprecision(4) << pose_3.p.x() << "/" << pose_4.p.x() << ") : y(" << pose_3.p.y() << "/" <<  pose_4.p.y() << ") : z(" << pose_3.p.z() << "/" << pose_4.p.z() << ") : q_x(" << x_3 << "/" << x_4 << ") : q_y(" << y_3 << "/" << y_4 << ") : q_z(" << z_3 << "/" << z_4 << ") : q_w(" << w_3 << "/" << w_4 << ")";
  ...
  pose_3.M.GetQuaternion(x_3, y_3, z_3, w_3);
  pose_4.M.GetQuaternion(x_4, y_4, z_4, w_4);
  EXPECT_TRUE(KDL::Equal(pose_3, pose_4, 0.001)) << "Inversion not correct with changed root: x(" << std::setprecision(4) << pose_3.p.x() << "/" << pose_4.p.x() << ") : y(" << pose_3.p.y() << "/" <<  pose_4.p.y() << ") : z(" << pose_3.p.z() << "/" << pose_4.p.z() << ") : q_x(" << x_3 << "/" << x_4 << ") : q_y(" << y_3 << "/" << y_4 << ") : q_z(" << z_3 << "/" << z_4 << ") : q_w(" << w_3 << "/" << w_4 << ")";  
  ...
  pose_3.M.GetQuaternion(x_3, y_3, z_3, w_3);
  pose_4.M.GetQuaternion(x_4, y_4, z_4, w_4);
  EXPECT_TRUE(KDL::Equal(pose_3, pose_4, 0.0001)) << "Triangulation not correct with changed root: x(" << std::setprecision(4) << pose_3.p.x() << "/" << pose_4.p.x() << ") : y(" << pose_3.p.y() << "/" <<  pose_4.p.y() << ") : z(" << pose_3.p.z() << "/" << pose_4.p.z() << ") : q_x(" << x_3 << "/" << x_4 << ") : q_y(" << y_3 << "/" << y_4 << ") : q_z(" << z_3 << "/" << z_4 << ") : q_w(" << w_3 << "/" << w_4 << ")";
}

TEST_F(KinematicaYoubotTest, CheckingKinematicsAgainstKDLNormalRoot)
{
  ...
  //!< Segment 1  
  ASSERT_GE(0, kdl_solver[SEGMENT_1]->JntToCart(joint_config_0_0_, kdl_pose)) << "Non-Kinematica Error: Could not compute FK from KDL";
  ...
  EXPECT_TRUE(KDL::Equal(kdl_pose, kinematica_pose, 0.001)) << "FK not correct for " << SEGMENT_1 << "! : x(" << std::setprecision(4) << kdl_pose.p.x() << "/" << kinematica_pose.p.x() << ") : y(" << kdl_pose.p.y() << "/" <<  kinematica_pose.p.y() << ") : z(" << kdl_pose.p.z() << "/" << kinematica_pose.p.z() << ") : q_x(" << kdl_x << "/" << kin_x << ") : q_y(" << kdl_y << "/" << kin_y << ") : q_z(" << kdl_z << "/" << kin_z << ") : q_w(" << kdl_w << "/" << kin_w << ")";
  
  //!< Segment 2  
  ASSERT_GE(0, kdl_solver[SEGMENT_2]->JntToCart(joint_config_0_3_, kdl_pose)) << "Non-Kinematica Error: Could not compute FK from KDL";
  ASSERT_TRUE(robot_tree_.getPose(SEGMENT_2, kinematica_pose));
  kdl_pose.M.GetQuaternion(kdl_x, kdl_y, kdl_z, kdl_w);
  kinematica_pose.M.GetQuaternion(kin_x, kin_y, kin_z, kin_w);
  EXPECT_TRUE(KDL::Equal(kdl_pose, kinematica_pose, 0.001)) << "FK not correct for " << SEGMENT_2 << "! : x(" << std::setprecision(4) << kdl_pose.p.x() << "/" << kinematica_pose.p.x() << ") : y(" << kdl_pose.p.y() << "/" <<  kinematica_pose.p.y() << ") : z(" << kdl_pose.p.z() << "/" << kinematica_pose.p.z() << ") : q_x(" << kdl_x << "/" << kin_x << ") : q_y(" << kdl_y << "/" << kin_y << ") : q_z(" << kdl_z << "/" << kin_z << ") : q_w(" << kdl_w << "/" << kin_w << ")";
  
  //!< Segment 3  
  ASSERT_GE(0, kdl_solver[SEGMENT_3]->JntToCart(joint_config_0_3_, kdl_pose)) << "Non-Kinematica Error: Could not compute FK from KDL";
  ASSERT_TRUE(robot_tree_.getPose(SEGMENT_3, kinematica_pose));
  kdl_pose.M.GetQuaternion(kdl_x, kdl_y, kdl_z, kdl_w);
  kinematica_pose.M.GetQuaternion(kin_x, kin_y, kin_z, kin_w);
  EXPECT_TRUE(KDL::Equal(kdl_pose, kinematica_pose, 0.001)) << "FK not correct for " << SEGMENT_3 << "! : x(" << std::setprecision(4) << kdl_pose.p.x() << "/" << kinematica_pose.p.x() << ") : y(" << kdl_pose.p.y() << "/" <<  kinematica_pose.p.y() << ") : z(" << kdl_pose.p.z() << "/" << kinematica_pose.p.z() << ") : q_x(" << kdl_x << "/" << kin_x << ") : q_y(" << kdl_y << "/" << kin_y << ") : q_z(" << kdl_z << "/" << kin_z << ") : q_w(" << kdl_w << "/" << kin_w << ")";
  
  //!< Segment 4  
  ASSERT_GE(0, kdl_solver[SEGMENT_4]->JntToCart(joint_config_0_8_, kdl_pose)) << "Non-Kinematica Error: Could not compute FK from KDL";
  ASSERT_TRUE(robot_tree_.getPose(SEGMENT_4, kinematica_pose));
  kdl_pose.M.GetQuaternion(kdl_x, kdl_y, kdl_z, kdl_w);
  kinematica_pose.M.GetQuaternion(kin_x, kin_y, kin_z, kin_w);
  EXPECT_TRUE(KDL::Equal(kdl_pose, kinematica_pose, 0.001)) << "FK not correct for " << SEGMENT_4 << "! : x(" << std::setprecision(4) << kdl_pose.p.x() << "/" << kinematica_pose.p.x() << ") : y(" << kdl_pose.p.y() << "/" <<  kinematica_pose.p.y() << ") : z(" << kdl_pose.p.z() << "/" << kinematica_pose.p.z() << ") : q_x(" << kdl_x << "/" << kin_x << ") : q_y(" << kdl_y << "/" << kin_y << ") : q_z(" << kdl_z << "/" << kin_z << ") : q_w(" << kdl_w << "/" << kin_w << ")";
...
TEST_F(KinematicaYoubotTest, CheckingKinematicsAgainstKDLAlternateRoot)
{
  ...
  //!< Segment 4  
  ASSERT_GE(0, kdl_solver[SEGMENT_4]->JntToCart(joint_config_0_8_, kdl_pose)) << "Non-Kinematica Error: Could not compute FK from KDL";
  ASSERT_GE(0, kdl_solver[SEGMENT_4]->JntToCart(joint_config_0_8_, kdl_pose_2, 0)) << "Non-Kinematica Error: Could not compute FK from KDL";
...
TEST_F(KinematicaYoubotTest, CheckingKinematicsAgainstKDLAlternateRootWithOffset)
{
  KDL::Frame kdl_pose, kdl_pose_2, kinematica_pose;
  double kdl_x, kdl_y, kdl_z, kdl_w, kin_x, kin_y, kin_z, kin_w;
...
TEST_F(KinematicaYoubotTest, TestingPhiComputation)
{
  //!< Temporaries
  ...
  EXPECT_TRUE(compareVectors(individual, forward_map, 0.0001)) << "The values do not match up";
...
TEST_F(KinematicaYoubotTest, TestingJacobianComputation)
{ 
  //!< Temporaries
  ...
  for (int i=0; i<NJOINTS; i++)
  {
    ...
    EXPECT_TRUE(compareVectors(diff_pose, jac_column, 0.0001)) << "The Jacobian entry for the perturbed joint " << i << " is wrong\n" << jac_column - diff_pose << "\n";
  }
  ...
  for (int i=0; i<NJOINTS; i++)
  {
    ...
    Eigen::VectorXd jac_column = jacobian.col(i); //!< Get the desired column (for the current joint)
    EXPECT_TRUE(compareVectors(diff_pose, jac_column, 0.00001)) << "The Jacobian entry for the perturbed joint " << i << " is wrong\n" << jac_column - diff_pose << "\n";
  }
  ...
  for (int i=0; i<NJOINTS; i++)
  {
    ...
    Eigen::VectorXd jac_column = jacobian.col(i); //!< Get the desired column (for the current joint)
    EXPECT_TRUE(compareVectors(diff_pose, jac_column, 0.0001)) << "The Jacobian entry for the perturbed joint " << i << " is wrong (Alternative Root/Random Vector)\n" << jac_column - diff_pose << "\n";
  }
  ...
  for (int i=0; i<NJOINTS; i++)
  {
    ...
    EXPECT_TRUE(compareVectors(Eigen::VectorXd::Zero(3), temp, 0.00001)) << " Jacobian Entry for " << SEGMENT_1 << " with itself as root should be 0!";
    temp = diff_pose.segment(0,3);
    EXPECT_TRUE(compareVectors(Eigen::VectorXd::Zero(3), temp, 0.00001)) << " Forward Kinematics Difference Entry for " << SEGMENT_1 << " with itself as root should be 0!";
  }
