			...
			for (std::size_t j = 0; j < index_size; j++) {
				...
				if (tmp_world->getObject(obj_id_[i])->shapes_[j]->type
					...
					if (tmp_world->getObject(obj_id_[i])->shapes_[j]->type
							!= shapes::MESH) {
						tmp_shape =
								boost::shared_ptr<const shapes::Shape>(
										shapes::createMeshFromShape(
...
EReturn CollisionScene::update(Eigen::VectorXdRefConst x) {
	...
	if (base_type_ == BASE_TYPE::FIXED) {
		...
		ps_->getCurrentStateNonConst().setVariablePosition(
				"world_joint/trans_x", x(0));
		ps_->getCurrentStateNonConst().setVariablePosition(
				"world_joint/trans_y", x(1));
		ps_->getCurrentStateNonConst().setVariablePosition(
				"world_joint/trans_z", x(2));
		...
		ps_->getCurrentStateNonConst().setVariablePosition("world_joint/x",
				x(0));
		ps_->getCurrentStateNonConst().setVariablePosition("world_joint/y",
				x(1));
...
EReturn CollisionScene::getCollisionLinkTranslation(const std::string & name,
	...
	if (fcl_robot_.find(name) == fcl_robot_.end())
		return FAILURE;
...
EReturn CollisionScene::getWorldObjectTranslation(const std::string & name,
		Eigen::Vector3d & translation) {
	if (fcl_world_.find(name) == fcl_world_.end())
		return FAILURE;
...
EReturn Scene::initialisation(tinyxml2::XMLHandle & handle,
	...
	if (!ok(
			server_->registerParam<std_msgs::String>(name_, tmp_handle,
					mode_))) {
...
EReturn Scene::activateTaskMaps() {
	...
	eff_index_.clear();
	for (auto & it : eff_names_) {
		eff_index_[it.first] = std::vector<int>(
				tmp_index.begin() + tmp_eff_size,
