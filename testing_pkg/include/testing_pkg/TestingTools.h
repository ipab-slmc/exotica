...
inline bool compareVectors(Eigen::VectorXdRefConst vec_1, Eigen::VectorXdRefConst vec_2, double prec=TOLERANCE)
{
  ...
  {
    if (std::abs(vec_1(i) - vec_2(i)) > prec) { return false; }
  }
  return true;
};


inline bool compareMatrices(const Eigen::MatrixXd & mat_1, const Eigen::MatrixXd & mat_2, double prec=TOLERANCE)
{
  if (mat_1.rows() != mat_2.rows() or mat_1.cols() != mat_2.cols()) { return false; }
  for (int i=0; i<mat_1.rows(); i++)
  {
    for (int j=0; j<mat_1.cols(); j++)
    {
      if (std::abs(mat_1(i,j) - mat_2(i,j)) > prec) { return false; }
    }
  }
  return true;
};
