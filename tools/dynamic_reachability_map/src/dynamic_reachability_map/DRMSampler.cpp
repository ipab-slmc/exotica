...
void DRMSampler::startSampling(DRMSpace_ptr &space, unsigned long int sample_cnt,
                               std::vector<std::vector<double>> &samples)
{
  ...
  for (unsigned int i = 0; i < space_->getSpaceSize(); i++)
  {
    ...
    volume_obj.header.frame_id = space->ps_->getRobotModel()->getRootLinkName();
  ...
  {
    ps_[t].reset(new planning_scene::PlanningScene(space_->ps_->getRobotModel()));
  ...
  for (int i = 0; i < thread_cnt_; i++)
    th[i] = new boost::thread(boost::bind(&DRMSampler::multiThreadSamplingFn, this, i));
  ...
  for (int t = 0; t < thread_cnt_; t++)
  {
    ...
    for (unsigned int i = 0; i < space_->getSpaceSize(); i++)
    {
      for (unsigned int l = 0; l < tmp_volumes_[t][i].occup_samples.size(); l++)
        space_->addOccupSample(i, tmp_volumes_[t][i].occup_samples[l]);
      for (unsigned int l = 0; l < tmp_volumes_[t][i].reach_samples.size(); l++)
...
std::vector<std::vector<double>> DRMSampler::preProcessSamples(std::vector<std::vector<double>> &samples)
{
  ...
  {
    std::cout << "Pre-process samples: (" << (double)i / (double)samples.size() * 100.0 << "%).\r";
    std::cout.flush();
    for (int n = 0; n < space_->getDimension(); n++)
      space_->ps_->getCurrentStateNonConst().setVariablePosition(space_->var_index_[n], samples[i][n]);
    space_->ps_->getCurrentStateNonConst().update(true);

    effpose = space_->ps_->getCurrentStateNonConst().getGlobalLinkTransform(space_->eff_);
    if (!space_->getVolumeIndex(effpose, index))
      continue;
  ...
  ROS_INFO_STREAM("Extracted "<<ret.size()<<" self-collision free samples from "<<samples.size()<<" samples");
  return ret;
}

unsigned long int DRMSampler::setToNextState(int id, Eigen::Affine3d &effpose, unsigned int &volume_index)
{
  boost::mutex::scoped_lock(curr_cnt_lock);
  unsigned long int index = curr_cnt_;
  if (index == sample_cnt_)
    return index + 1;
  ...
  if (samples_.size() > 0)
  {
    for (int i = 0; i < space_->getDimension(); i++)
      ps_[id]->getCurrentStateNonConst().setVariablePosition(space_->var_index_[i], samples_[index][i]);
    ps_[id]->getCurrentStateNonConst().update(true);
    effpose = ps_[id]->getCurrentStateNonConst().getGlobalLinkTransform(space_->eff_);
...
void DRMSampler::multiThreadSamplingFn(int id)
{
  ...
  while (true)
  {
    unsigned long int sample_index = setToNextState(id, effpose, volume_index);
    if (sample_index >= sample_cnt_)
      break;
    if (id == 0)
    {
      std::cout << "Sampling process [" << id << "]: (" << (double)sample_index / (double)sample_cnt_ * 100.0 << "%).\r";
      std::cout.flush();
    }
    ...
    if (result.collision)
    {
      for (auto &it : result.contacts)
      {
        ...
        {
          ROS_ERROR_STREAM("This should not happen! Contact between "<<it.first.first<<" and "<<it.first.second);
          continue;
        }
...
//          space_->addOccupSample(tmp, sample_index);
    ...
    for (int i = 0; i < space_->var_index_.size(); i++)
      space_->samples_[sample_index].q[i] = ps_[id]->getCurrentState().getVariablePosition(space_->var_index_[i]);
