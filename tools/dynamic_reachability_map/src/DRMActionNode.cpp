...
DRMActionNode::DRMActionNode() :
    nh_("~"), as_(nh_, "/DRM_IK", boost::bind(&dynamic_reachability_map::DRMActionNode::getIKSolution, this, _1), false)
{
}
...
bool DRMActionNode::initialise()
{
  ...
  if (nh_.hasParam("Clustering"))
    nh_.getParam("Clustering", clustering);
  if (clustering)
  {
...
//  drm_state_timer_ = nh_.createTimer(ros::Duration(1), &DRMActionNode::drmStateTimeCallback, this);
  ...
  traj_pub_ = nh_.advertise<moveit_msgs::DisplayTrajectory>("ClusterCenters", 1);
  disp_traj_.model_id = drm_->space()->getPlanningScene()->getRobotModel()->getName();
  disp_traj_.trajectory.resize(1);
  disp_traj_.trajectory[0].joint_trajectory.header.frame_id = "world_frame";
  disp_traj_.trajectory[0].joint_trajectory.joint_names = drm_->space()->getGroup()->getVariableNames();

  astar_traj_ = disp_traj_;
  astar_pub_ = nh_.advertise<moveit_msgs::DisplayTrajectory>("AStarTrajectory", 1);
  ...
  cell_ps_pub_ = nh_.advertise<moveit_msgs::PlanningScene>("CellPlanningScene", 10);
  state_pub_ = nh_.advertise<moveit_msgs::DisplayRobotState>("DRMState", 10);
  drm_pub_ = nh_.advertise<visualization_msgs::Marker>("DynamicReachabilityMap", 10);
  ...
  drm_mark_.scale.x = drm_mark_.scale.y = drm_mark_.scale.z = drm_->space()->getResolution();
  ...
  graph_markers_.markers[0].scale.x = graph_markers_.markers[0].scale.y = 0.002;
  ...
  graph_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("ConnectivityGraph", 1);
  ...
  {
    graph_markers_.markers[0].points[i] = drm_->space()->getSample(i).effpose.position;
  ...
  {
    graph_markers_.markers[1].points[2 * i] = drm_->space()->getSample(drm_->space()->edges_[i].a).effpose.position;
    graph_markers_.markers[1].points[2 * i + 1] = drm_->space()->getSample(drm_->space()->edges_[i].b).effpose.position;
...
bool DRMActionNode::updateDRM(const dynamic_reachability_map::DRMGoalConstPtr &goal, const KDL::Frame &base_pose)
{
  ...
  for (unsigned int i = 0; i < drm_->space()->getSpaceSize(); i++)
  {
    ...
    cell_ps_->getCurrentStateNonConst().setVariablePosition(0, cell_pose.p.x());
    cell_ps_->getCurrentStateNonConst().setVariablePosition(1, cell_pose.p.y());
    cell_ps_->getCurrentStateNonConst().setVariablePosition(2, cell_pose.p.z());
...
bool DRMActionNode::getIKSolution(const dynamic_reachability_map::DRMGoalConstPtr &goal)
{
  ...
  if (false && result.succeed)
  {
    ...
    {
      ROS_INFO_STREAM("A* succeeded in "<<ros::Duration(ros::Time::now()-astar_start).toSec()<<" path length "<<path.size());
      ...
      astar_traj_.trajectory[0].joint_trajectory.header.stamp = ros::Time::now();
      astar_traj_.trajectory[0].joint_trajectory.points.resize(path.size());
      for (int i = 0; i < path.size(); i++)
      {
        astar_traj_.trajectory[0].joint_trajectory.points[i].positions.resize(drm_->space()->getDimension());
        for (int j = 0; j < drm_->space()->getDimension(); j++)
        {
          astar_traj_.trajectory[0].joint_trajectory.points[i].positions[j] = drm_->space()->getSample(path[i]).q[j];
        }
      }
      astar_traj_.trajectory_start.joint_state.name = astar_traj_.trajectory[0].joint_trajectory.joint_names;
      astar_traj_.trajectory_start.joint_state.position = astar_traj_.trajectory[0].joint_trajectory.points[path.size()
          - 1].positions;
...
void DRMActionNode::drmStateTimeCallback(const ros::TimerEvent& event)
{
  ...
  if (space_index == drm_->space()->getSpaceSize())
    space_index = 0;
  ...
  disp_traj_.trajectory[0].joint_trajectory.points.resize(drm_->space()->at(space_index).reach_samples.size());
  for (int i = 0; i < drm_->space()->at(space_index).reach_samples.size(); i++)
  {
    disp_traj_.trajectory[0].joint_trajectory.points[i].positions.resize(drm_->space()->getDimension());
    for (int j = 0; j < drm_->space()->getDimension(); j++)
    {
      disp_traj_.trajectory[0].joint_trajectory.points[i].positions[j] = drm_->space()->getSample(
          drm_->space()->at(space_index).reach_samples[i]).q[j];
    }
  }
  disp_traj_.trajectory_start.joint_state.name = disp_traj_.trajectory[0].joint_trajectory.joint_names;
  disp_traj_.trajectory_start.joint_state.position = disp_traj_.trajectory[0].joint_trajectory.points[drm_->space()->at(
...
void DRMActionNode::graphTimeCallback(const ros::TimerEvent& event)
{
  for (unsigned long int i = 0; i < drm_->space()->getSampleSize(); i++)
  {
    graph_markers_.markers[0].colors[i].a = drm_->space()->getSample(i).isValid ? 1 : 0;

  }

  for (unsigned long int i = 0; i < drm_->space()->edges_.size(); i++)
  {
    graph_markers_.markers[1].colors[2 * i].a = graph_markers_.markers[1].colors[2 * i + 1].a =
