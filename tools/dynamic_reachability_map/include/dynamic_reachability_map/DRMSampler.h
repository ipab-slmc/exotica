...
public:
  ...
  void startSampling(DRMSpace_ptr &space, unsigned long int sample_cnt, std::vector<std::vector<double>> &samples);
private:
  void multiThreadSamplingFn(int id);
  std::vector<std::vector<double>> preProcessSamples(std::vector<std::vector<double>> &samples);
  unsigned long int setToNextState(int id, Eigen::Affine3d &effpose, unsigned int &volume_index);
  DRMSpace_ptr space_;
