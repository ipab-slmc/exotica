...
namespace exotica
{
	...
	OMPLsolver::OMPLsolver () : timeout_(60.0), finishedSolving_(false)
	{
	...
	EReturn OMPLsolver::Solve(Eigen::VectorXd q0, Eigen::MatrixXd & solution)
	{
		...
		if(ok(state_space_->copyToOMPLState(ompl_start_state.get(),q0) ))
		{
			...
			if(ompl_simple_setup_->solve(ptc) == ompl::base::PlannerStatus::EXACT_SOLUTION)
			{
				...
				convertPath(ompl_simple_setup_->getSolutionPath(),solution);
				return SUCCESS;
			}
			else
			{
	...
	EReturn OMPLsolver::convertPath(const ompl::geometric::PathGeometric &pg, Eigen::MatrixXd & traj)
	{
	...
	EReturn OMPLsolver::initDerived(tinyxml2::XMLHandle & handle)
	{
		...
		XML_CHECK("max_goal_sampling_attempts"); XML_OK(getInt(*xmltmp,goal_ampling_max_attempts_));
		return SUCCESS;
	}
	...
	template<typename T> static ompl::base::PlannerPtr allocatePlanner(const ob::SpaceInformationPtr &si, const std::string &new_name)
	{
