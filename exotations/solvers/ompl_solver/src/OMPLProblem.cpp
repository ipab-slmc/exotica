...
namespace exotica
{
			...
			boost::shared_ptr<PlanningProblem> problem)
	{
		...
		if (document.IsArray())
		{
			...
			if (!problem->endStateName.empty() && problemType == OMPL_PROBLEM_GOAL_BIAS)
			{
				...
				if (ok(TaskMap_fac::Instance().createObject("Identity", taskmap)))
				{
					...
					EReturn ret1 =
							taskmap->initialiseManual("exotica::Identity", server_, scenes_, problem);
					EReturn ret =
				...
				&& server_->hasParam(server_->getName() + "/FloatingBaseUpperLimits"))
		{
			...
			if (ok(server_->getParam(server_->getName() + "/FloatingBaseLowerLimits", tmp_lower))
					&& tmp_lower->data.size() == 6
					&& ok(server_->getParam(server_->getName() + "/FloatingBaseUpperLimits", tmp_upper))
					&& tmp_upper->data.size() == 6)
			{
				...
				scenes_.begin()->second->getSolver().setFloatingBaseLimitsPosXYZEulerZYX(lower, upper);
			}
			else
			{
				INDICATE_FAILURE
