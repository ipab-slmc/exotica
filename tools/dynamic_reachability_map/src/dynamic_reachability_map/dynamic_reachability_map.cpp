...
bool DynamicReachabilityMap::createSpace(const Eigen::Vector2d & bx, const Eigen::Vector2d & by,
...
bool DynamicReachabilityMap::createRobotSampler(const robot_model::RobotModelConstPtr &model, const std::string & eff,
                                                const std::string & group_name)
{
  ...
  std::vector<std::string> joint_names = ps_->getCurrentState().getVariableNames();
...
bool DynamicReachabilityMap::isStateValid(const ompl::base::State *state)
{
  ...
  for (int i = 0; i < dimension_; i++)
    ps_->getCurrentStateNonConst().setVariablePosition(joint_names_[i], eigen(i));
  ps_->getCurrentStateNonConst().update(true);
  bool valid = ps_->isStateValid(ps_->getCurrentState());
  return valid;
}

void DynamicReachabilityMap::pathSimplifier(const Eigen::MatrixXd & path, Eigen::MatrixXd &simple)
{

}

bool DynamicReachabilityMap::createSpacePlanningScene(moveit_msgs::PlanningSceneWorld &world,
...
bool DynamicReachabilityMap::setSpacePlanningScene(const moveit_msgs::PlanningSceneWorld world)
{
...
bool DynamicReachabilityMap::indexingSpace(int sample_cnt, std::map<std::string, int> cell_name_map)
{
  ...
  {
    if (sample_size_ < 1000 || (thread_id_ == 0 && i % (sample_size_ / 1000) == 0))
    {
      std::cout << "Sampling process " << i << " / " << sample_size_ << "(" << (double)i / (double)sample_size_ * 100.0
          << "%).\r";
      std::cout.flush();
    }
    bool valid = false;
    ...
    while (!valid)
    {
      ...
      ps_->getCurrentStateNonConst().update(true);
      Eigen::Affine3d effpose = ps_->getCurrentStateNonConst().getGlobalLinkTransform(eff_);
    ...
    ps_->getCurrentStateNonConst().update(true);
    Eigen::Affine3d effpose = ps_->getCurrentStateNonConst().getGlobalLinkTransform(eff_);
    if (effpose.translation()(0) < bounds_[0](0) || effpose.translation()(0) > bounds_[0](1)
        || effpose.translation()(1) < bounds_[1](0) || effpose.translation()(1) > bounds_[1](1)
        || effpose.translation()(2) < bounds_[2](0) || effpose.translation()(2) > bounds_[2](1))
    {
      --i;
      continue;
    }
...
//				ROS_WARN_STREAM("TCP index "<<tcp_index<<" out of bounds "<<space_size_);
    ...
    {
      samples_[i].q(var) = ps_->getCurrentState().getVariablePosition(group_->getVariableNames()[var]);
    }
  }
...
bool DynamicReachabilityMap::update(const geometry_msgs::Point & point, int &index)
{
  ...
  if (index < 0 || index > space_size_)
    return true;
  if (space_[index].space_index(0) != x || space_[index].space_index(1) != y || space_[index].space_index(2) != z)
  {
...
bool DynamicReachabilityMap::updatePlanningScene(const geometry_msgs::Point & point, int &index)
{
  int x = (point.x - bounds_[0](0)) / cell_size_;
  int y = (point.y - bounds_[1](0)) / cell_size_;
  int z = (point.z - bounds_[2](0)) / cell_size_;
...
void DynamicReachabilityMap::getSpaceCentres(std::vector<geometry_msgs::Point> & centres)
{
...
bool DynamicReachabilityMap::solve(const Eigen::VectorXd & q0, const geometry_msgs::Pose & goal,
                                   ...
                                   std::vector<int> & sample_path)
{
  ...
  {
    ROS_ERROR_STREAM("Configuration space dim mismatch "<<q0.rows() <<" ! = "<<dimension_);
    return false;
  }
  ...
  if (effpose.translation()(0) < bounds_[0](0) || effpose.translation()(0) > bounds_[0](1)
      ...
      || effpose.translation()(2) < bounds_[2](0) || effpose.translation()(2) > bounds_[2](1))
  {
    ROS_ERROR_STREAM("Start TCP "<<effpose.translation().transpose()<<" is not in the reachable space");
    return false;
  }
  //	Find out which cell the tcp lies in
  int x = (effpose.translation()(0) - bounds_[0](0)) / cell_size_;
  ...
  if (space_[tcp_index].space_index(0) != x || space_[tcp_index].space_index(1) != y
  ...
  {
    ROS_ERROR_STREAM("Goal index "<<goal_index<<" out of bounds "<<space_size_);
    return false;
  }
  if (space_[goal_index].space_index(0) != x || space_[goal_index].space_index(1) != y
  ...
  for (int i = 0; i < sample_size; i++)
  {
    ...
    {
      double tmp_dist = (samples_[tcps_[tcp_index].samples[i]].q - current).cwiseAbs().maxCoeff();
      if (tmp_dist < dist)
      {
        dist = tmp_dist;
  ...
  for (std::map<int, double>::iterator it = neighbors.begin(); it != neighbors.end(); it++)
  {
    ...
    for (int j = 0; j < cell_sample_cnt; j++)
    {
      ...
      {
        double tmp_dist = (samples_[tcps_[it->first].samples[j]].q - current).cwiseAbs().maxCoeff();
        if (tmp_dist < dist)
        {
          dist = tmp_dist;
  ...
  if (succeed)
  {
    ROS_INFO_STREAM("A* succeeded in "<<ros::Duration(ros::Time::now()-start_time).toSec()<<"sec");
    ...
    for (int i = 0; i < path.size(); i++)
    {
      ...
      for (int j = 0; j < cell_sample_cnt; j++)
      {
        ...
        {
          double tmp_dist = (samples_[tcps_[path[i]].samples[j]].q - current).cwiseAbs().maxCoeff();
          if (tmp_dist < dist)
          {
            dist = tmp_dist;
...
{
  double weight = 1e3;
  double tmp = (space_[b].space_index - space_[a].space_index).cwiseAbs2().sum();
  return weight * sqrt(tmp);
}

std::map<int, double> DynamicReachabilityMap::getNeighbors(int index)
{
  static std::vector<std::pair<Eigen::Vector3i, double> > offsets = getNeighborOffsets();
  std::map<int, double> Neighbors;
  for (int i = 0; i < offsets.size(); i++)
  {
    Eigen::Vector3i tmp_space_index = space_[index].space_index + offsets[i].first;
    int tmp_index = tmp_space_index(0) + tmp_space_index(1) * cell_cnts_(0)
        + tmp_space_index(2) * cell_cnts_(0) * cell_cnts_(1);
    if (tmp_index < 0 || tmp_index >= space_size_ || tmp_space_index(0) < 0 || tmp_space_index(1) < 0
        || tmp_space_index(2) < 0 || tmp_space_index(0) >= cell_cnts_(0) || tmp_space_index(1) >= cell_cnts_(1)
...
//				ROS_ERROR_STREAM("Index out of bounds "<<tmp_index<<" >= "<<space_size_);
    ...
    {
      Neighbors[tmp_index] = density_[tmp_index] * getHeuristicDist(index, tmp_index);
    }
  }
  return Neighbors;
}

bool DynamicReachabilityMap::AStarPathPlanning(int start, const Eigen::VectorXd &start_q, int goal,
                                               std::vector<int> &path)
{
///	Lets do A* !
  ...
  while (open.size() > 0)
  {
    ...
    for (std::map<int, double>::iterator it = open.begin(); it != open.end(); it++)
    {
      if (it->second < min)
        current = it;
    }
    ...
    for (std::map<int, double>::iterator it = neighbors.begin(); it != neighbors.end(); it++)
    {
      ...
      else
      {
        ...
        if (open.find(it->first) == open.end() || tentative_g < aStar[it->first].g)
        {
    ...
    const dynamic_reachability_map::DRMGoalConstPtr &goal)
{
  ...
  if (goal->goal_pose.position.x < bounds_[0](0) || goal->goal_pose.position.x > bounds_[0](1)
      || goal->goal_pose.position.y < bounds_[1](0) || goal->goal_pose.position.y > bounds_[1](1)
      || goal->goal_pose.position.z < bounds_[2](0) || goal->goal_pose.position.z > bounds_[2](1))
  {
    ROS_ERROR_STREAM(
  ...
  if (tcp_index < 0 || tcp_index >= space_size_)
  {
    ROS_ERROR_STREAM("Goal index "<<tcp_index<<" out of bounds "<<space_size_);
    return result;
  }
  if (space_[tcp_index].space_index(0) != x || space_[tcp_index].space_index(1) != y
      || space_[tcp_index].space_index(2) != z)
  ...
  if (density_[tcp_index] <= 0)
  {
    ROS_ERROR_STREAM("Cell "<<tcp_index<<" is not reachable in current environment");
    return result;
  }
  double dist = INFINITY;
  int tmp_index = -1;
...
bool DynamicReachabilityMap::checkPathDetail(const std::vector<int> &sample_path, std::pair<int, int> &disconnect)
{
  bool valid = true;
  bool isFirst = true;
  for (int i = 0; i < sample_path.size(); i++)
...
bool DynamicReachabilityMap::checkPathUsePlanningScene(const Eigen::MatrixXd &solution)
{
  bool valid = true;
  for (int i = 0; i < solution.rows(); i++)
  {
    for (int j = 0; j < dimension_; j++)
      ps_->getCurrentStateNonConst().setVariablePosition(joint_names_[j], solution(i, j));
    ps_->getCurrentStateNonConst().update(true);
    if (!ps_->isStateValid(ps_->getCurrentState()))
      return false;
  }
  return valid;
}
...
{
  if (point.x < bounds_[0](0) || point.x > bounds_[0](1) || point.y < bounds_[1](0) || point.y > bounds_[1](1)
      || point.z < bounds_[2](0) || point.z > bounds_[2](1))
    return -1;
  int x = (point.x - bounds_[0](0)) / cell_size_;
  int y = (point.y - bounds_[1](0)) / cell_size_;
  int z = (point.z - bounds_[2](0)) / cell_size_;
...
{
  ROS_WARN_STREAM("max dist tol="<<h<<", sqrt dist tol="<<sqrt(dimension_ * (h * h)));
  ...
  for (int i = 0; i < clusters.size(); i++)
  {
    ...
    {
      for (int k = 0; k < sample_occupation_[clusters[i].samples[j]].size(); k++)
        if (tmp.find(sample_occupation_[clusters[i].samples[j]][k]) == tmp.end())
          tmp[sample_occupation_[clusters[i].samples[j]][k]] = sample_occupation_[clusters[i].samples[j]][k];
    }
  }
  return true;
}

bool DynamicReachabilityMap::meanShiftCell(int index, double h, std::vector<Cluster> &clusters)
{
  ...
  while (sample_cnt > 0)
  {
    ...
    while (true)
    {
      ...
      if (open_samples.find(tmp_index) == open_samples.end())
        continue;
    ...
    while (iteration < 200)
    {
      ...
      for (std::map<int, int>::iterator it = open_samples.begin(); it != open_samples.end(); it++)
      {
        double max_dist = (my_mean - samples_[it->second].q).cwiseAbs().maxCoeff();
//        double sqr_dist = (my_mean - samples_[it->second].q).norm();
      ...
      if (small_cluster || (my_mean - old_mean).norm() < tol)
      {
        ...
        if (merge_index != -1)
        {
          tmp_clusters[merge_index].center = (tmp_clusters[merge_index].center + my_mean) / 2;
          for (int i = 0; i < myMembers.size(); i++)
  ...
  ROS_INFO_STREAM("Remove small clusters from "<<tmp_clusters.size()<<" clusters");
  std::vector<int> big_index;
  for (int i = 0; i < tmp_clusters.size(); ++i)
    if (tmp_clusters[i].samples.size() >= 20)
  ...
  for (int i = 0; i < clusters.size(); i++)
  {
    ...
    ROS_INFO_STREAM("Cluster "<<i<<" has "<<clusters[i].samples.size()<<" samples");
    sample_cnt += clusters[i].samples.size();
  }
  ROS_INFO_STREAM(clusters.size()<<" clusters remained, with "<<sample_cnt<<" samples");
  return true;
}

bool DynamicReachabilityMap::saveSpace(const std::string & path, int multiThreads)
{
  struct stat st = {0};
  std::string folderpath = path + "/" + ps_->getRobotModel()->getName() + "_" + std::to_string(samples_.size()) + "_"
      + std::to_string(cell_size_);
  if (stat(folderpath.c_str(), &st) == -1 && mkdir(folderpath.c_str(), 0700) != 0)
  {
  ...
  {
    samples_file << samples_[i].tcp.position.x << " " << samples_[i].tcp.position.y << " " << samples_[i].tcp.position.z
        << " " << samples_[i].tcp.orientation.x << " " << samples_[i].tcp.orientation.y << " "
        << samples_[i].tcp.orientation.z << " " << samples_[i].tcp.orientation.w << " ";
  ...
  for (int i = 0; i < space_size_; i++)
  {
    space_file << space_[i].centre.x << " " << space_[i].centre.y << " " << space_[i].centre.z << " "
        << space_[i].space_index(0) << " " << space_[i].space_index(1) << " " << space_[i].space_index(2) << std::endl;
  }
  space_file.close();
  ...
  for (int t = 0; t < multiThreads; t++)
  {
    ...
    space_occupied_file.open(folderpath + "/space_occupation/space_occupation_" + std::to_string(t) + ".txt");
    for (int i = t * tmp_space_size; i < (t + 1) * tmp_space_size; i++)
    {
      ROS_INFO_STREAM_THROTTLE(2, "Space "<<i<<" / "<<space_size_<<"("<<(double)i/(double)space_size_*100.0<<"%).");
...
bool DynamicReachabilityMap::loadSpace(const std::string & path)
{
  ...
  {
    if (files.find(dirp->d_name) != files.end())
      files.at(dirp->d_name) = true;
  }
  for (auto &it : files)
    if (!it.second)
    {
...
//  Joint names
  ...
  if (joint_names.size() != dimension_ || joint_names.size() != group_->getVariableCount())
    ROS_ERROR_STREAM(
...
//  Threads
  ...
  {
    ROS_ERROR_STREAM("Result directory "<<path<<"/space_occupation does not exist");
    return false;
  }
...
///	Loading occupation
  MultiThreadsSpaceOccupationLoader loader(path + "/space_occupation", thread_cnt, space_size_, sample_size_);
  loader.loadSpaceOccupation();
  int tmp_space_size = space_size_ / thread_cnt;
  for (int t = 0; t < thread_cnt; t++)
    for (int i = 0; i < tmp_space_size; i++)
      space_[t * tmp_space_size + i].occupiedSmaples = loader.space_occup_[t][i];
...
///	Loading TCP
  ...
  ROS_INFO_STREAM("Loaded Indexed Space created with "<<space_size_<<" cells, "<<sample_size_<<" samples.");
  return true;
}
...
bool DynamicReachabilityMap::createCellURDF(const std::string & path)
{
  ...
  for (int i = 0; i < space_size_; i++)
  {
    ...
    urdf << "\t\t<origin rpy=\"0 0 0\" xyz=\"" << space_[i].centre.x << " " << space_[i].centre.y << " "
        << space_[i].centre.z << "\"/>" << std::endl;
    urdf << "\t\t<geometry>" << std::endl;
    urdf << "\t\t\t<box size=\"" << cell_size_ << " " << cell_size_ << " " << cell_size_ << "\"/>" << std::endl;
    urdf << "\t\t</geometry>" << std::endl;
    urdf << "\t\t<material name=\"Black\"/>" << std::endl;
    urdf << "\t</visual>" << std::endl;
...
{
  std::ifstream space_occupied_file;
  space_occupied_file.open(path_ + "/space_occupation_" + std::to_string(id) + ".txt");
  ...
  for (int i = 0; i < space_size_; i++)
  {
    ...
    {
      std::cout << "Load space " << i << " / " << space_size_ << "(" << (double)i / (double)sample_size_ * 1000.0
          << "%).\r";
      std::cout.flush();
    }
