...
class EffPublisher
{
  public:
        ...
        : nh_("/OMPLEffTrajectory"), links_(links)
    {
      ...
      for (int i = 0; i < links.size(); i++)
        ...
        {
          WARNING_NAMED("OMPLEffTrajectory",
              "End-effector "<<links[i]<<" does not exist.")
          ok_ = false;
          break;
        }
      ...
      if (ok_)
      {
        sub_ = nh_.subscribe<moveit_msgs::DisplayTrajectory>(
            "/move_group/display_planned_path", 1,
            boost::bind(&EffPublisher::pathCallback, this, _1));
        HIGHLIGHT_NAMED("OMPLEffTrajectory",
            "Subscribing to path topic '/move_group/display_planned_path'");
        ...
        for (int i = 0; i < links.size(); i++)
        {
          ...
          HIGHLIGHT_NAMED("OMPLEffTrajectory",
              "Publishing trajectory of "<<links_[i]<<" to topic '/OMPLEffTrajectory/"<<links_[i]<<"'.");
      ...
      traj_pub_ = nh_.advertise<moveit_msgs::DisplayTrajectory>("ShiftedPath",
          1);
    }
  ...
  private:
    void pathCallback(const moveit_msgs::DisplayTrajectoryConstPtr & path)
    {
      if (ok_)
      {
        INFO_NAMED("OMPLEffTrajectory",
            "Publishing new end-effector trajectories");
        for (int l = 0; l < links_.size(); l++)
        {
          ...
          {
            for (int j = 0;
                j < path->trajectory[0].joint_trajectory.joint_names.size();
                j++)
              state_->setVariablePosition(
                  path->trajectory[0].joint_trajectory.joint_names[j],
                  path->trajectory[0].joint_trajectory.points[i].positions[j]);
            state_->update(true);
            Eigen::Vector3d pose =
        ...
        shift_traj_.trajectory[0].multi_dof_joint_trajectory.points.resize(
            size);
        ...
        for (int i = 0; i < size; i++)
        {
          ...
          shift_traj_.trajectory[0].multi_dof_joint_trajectory.points[i].transforms.resize(
              1);
...
int main(int argc, char **argv)
{
  ...
  {
    HIGHLIGHT_NAMED("OMPLEffTrajectory",
        "Please at least specify one end-effector link name");
    return 0;
  }
