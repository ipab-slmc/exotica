...
TEST_F(ExoticaTaskTest, RCT) //!< Registration and Creation Test
{
  ...
  {
    if (task1_name.compare(registered_types_[i]) == 0) {task1_ok = true;}
    if (task2_name.compare(registered_types_[i]) == 0) {task2_ok = true;}
  }
  if (!task1_ok)
  {
  ...
  task1_p_ = boost::dynamic_pointer_cast<ExoticaTaskTest_1>(exotica::TaskCreator::Instance()->createObject("ExoticaTask_3", params_));
  ASSERT_TRUE(task1_p_ == nullptr);
  task1_p_ = boost::dynamic_pointer_cast<ExoticaTaskTest_1>(exotica::TaskCreator::Instance()->createObject("ExoticaTask_1", params_));
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object 1";
  task2_p_ = boost::dynamic_pointer_cast<ExoticaTaskTest_2>(exotica::TaskCreator::Instance()->createObject("ExoticaTask_2", params_));
  ASSERT_TRUE(task2_p_ != nullptr) <<" : Could not initialise object 2";
  
  std::string testing;
  EXPECT_TRUE(exotica::TestRegistrar::Instance()->findXML("ExoticaTask_1", testing)) << " : Tests for Task 1 not registered";
  EXPECT_TRUE(exotica::TestRegistrar::Instance()->findXML("ExoticaTask_2", testing)) << " : Tests for Task 2 not registered";
  EXPECT_FALSE(exotica::TestRegistrar::Instance()->findXML("ExoticaTask_3", testing)) << " : Somehow registered for incorrect name";
...
TEST_F(ExoticaTaskTest, MIT) //!< Manual Initialisation Test
{
  ...
  EXPECT_EQ(1, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
  ...
  task1_p_ = boost::dynamic_pointer_cast<ExoticaTaskTest_1>(exotica::TaskCreator::Instance()->createObject("ExoticaTask_1", params_));
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object";
  EXPECT_EQ(3, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
...
TEST_F(ExoticaTaskTest, XIT) //!< XML Initialisation Test
{
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_1.xml")).c_str()));
  ...
  task1_p_ = boost::dynamic_pointer_cast<ExoticaTaskTest_1>(exotica::TaskCreator::Instance()->createObject("ExoticaTask_1", params_));
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object";
  EXPECT_EQ(2, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ExoticaTask2_default.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  ...
  EXPECT_EQ(1, task2_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
...
TEST_F(ExoticaTaskTest, BXT) //!< Broken XML file Test
{
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_2.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_10.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_FALSE(task1_p_->initBase(*xml_handle));
  EXPECT_FALSE(task1_p_->derived_called);
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_11.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_FALSE(task1_p_->initBase(*xml_handle));
  EXPECT_FALSE(task1_p_->derived_called);
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_3.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_FALSE(task1_p_->initBase(*xml_handle));
  EXPECT_FALSE(task1_p_->derived_called);
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_12.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_FALSE(task1_p_->initBase(*xml_handle));
  EXPECT_FALSE(task1_p_->derived_called);
...
/*ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile(resource_path_ + std::string("ex_task_13.xml").c_str()));
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_14.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_FALSE(task1_p_->initBase(*xml_handle));
  EXPECT_FALSE(task1_p_->derived_called);
  ...
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_4.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ...
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object";
  EXPECT_EQ(2, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_8.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_FALSE(task1_p_->initBase(*xml_handle));
  EXPECT_FALSE(task1_p_->derived_called);
  ...
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object";
  EXPECT_EQ(2, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_9.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_FALSE(task1_p_->initBase(*xml_handle));
  EXPECT_FALSE(task1_p_->derived_called);
...
TEST_F(ExoticaTaskTest, IXT)  //!< Incomplete XML Test (or reordering)
{
  ...
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object";
  EXPECT_EQ(1, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_5.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_TRUE(task1_p_->initBase(*xml_handle));
  EXPECT_TRUE(task1_p_->derived_called);
  ...
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object";
  EXPECT_EQ(1, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_6.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_TRUE(task1_p_->initBase(*xml_handle));
  EXPECT_TRUE(task1_p_->derived_called);
  ...
  ASSERT_TRUE(task1_p_ != nullptr) <<" : Could not initialise object";
  EXPECT_EQ(1, task1_p_->getParameters().optimisation_window) << " : Incorrect initialisation";
  ASSERT_EQ(tinyxml2::XML_NO_ERROR, doc.LoadFile((resource_path_ + std::string("ex_task_7.xml")).c_str()));
  xml_handle.reset(new tinyxml2::XMLHandle(*(doc.RootElement())));
  ASSERT_NE(nullptr, xml_handle) << " : Aborting: could  not initialise";
  EXPECT_TRUE(task1_p_->initBase(*xml_handle));
  EXPECT_TRUE(task1_p_->derived_called);
...
TEST_F(ExoticaTaskTest, DTT)  //!< Derived-task Jacobian Tests: will contain tests to check whether the jacobian is correctly computed through finite-differences method
{
  ...
  for (int i=0; i<registered_types_.size(); i++)  //!< Iterate through the registered tasks
  {
    ...
    {
      if (document.LoadFile((resource_path_ + xml_path).c_str()) != tinyxml2::XML_NO_ERROR) //!< Attempt to load file
      {
      ...
      while (element_ptr->ToElement()) //!< Iterate through all possible test cases
      {
        ...
        for (int k=0; k<conf_space.size(); k++)
        {
          ...
          if (!task_ptr->updateTask(conf_perturb, 0)) { ADD_FAILURE() << " for " << registered_types_[i] << " @ iteration " << j << " in config-dimension " << k; continue; }
          if (task_ptr->getPhi(0).size() != phi.size()) { ADD_FAILURE() << " for " << registered_types_[i] << " @ iteration " << j << " in config-dimension " << k; continue; }
