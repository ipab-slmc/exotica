...
namespace exotica
{
	...
	EReturn SweepFlux::setTimeSteps(const int T)
	{
		...
		for (int t = 0; t < (capEnds_->data ? T_ : (T_ - 1)); t++)
		{
			...
			if (capBottom_->data)
			{
				b = t * (scene_->getMapSize(object_name_)) + scene_->getMapSize(object_name_) - 1;
				a = ((t + 1) % T_) * (scene_->getMapSize(object_name_)) + scene_->getMapSize(object_name_) - 1;
				c = scene_->getMapSize(object_name_) - 1;
				Tris_(((capTop_->data ? 1 : 0) + t * TrisStride_ + (scene_->getMapSize(object_name_) - 1) * 2)
						* 3) = a;
				Tris_(((capTop_->data ? 1 : 0) + t * TrisStride_ + (scene_->getMapSize(object_name_) - 1) * 2)
						* 3 + 1) = b;
				Tris_(((capTop_->data ? 1 : 0) + t * TrisStride_ + (scene_->getMapSize(object_name_) - 1) * 2)
	...
	void SweepFlux::doVis()
	{
		...
		for (int i = 0; i < Tris_.rows() / 3; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				marker.points[TrisQ_.rows() + i * 3 + j + Tris_.rows()].x = Verts_(Tris_(i * 3 + j)
						* 3) - tmp(0);
				marker.points[TrisQ_.rows() + i * 3 + j + Tris_.rows()].y = Verts_(Tris_(i * 3 + j)
						* 3 + 1) - tmp(1);
				marker.points[TrisQ_.rows() + i * 3 + j + Tris_.rows()].z = Verts_(Tris_(i * 3 + j)
						* 3 + 2) - tmp(2);
	...
	void SweepFlux::FluxTriangleTriangle(int* Tris, double* Verts, double* VertJ, int* TrisJ,
			int* TrisQ, double* VertsQ, double* Flux, double* FluxJ, int nTris, int nTrisJ, int n,
			int nTrisQ, double* FluxQ)
	{
		memset(Flux, 0, sizeof(double) * nTris);
		memset(FluxJ, 0, sizeof(double) * n);
		double *a, *b, *c, *d, *e, *f, tmp[3], m1[3], m2[3], m3[3], m4[3], *aJ, *bJ, *cJ, *tmpFluxJ,
				area, f0, f1, f2, f3;
		aJ = new double[3 * n];
		bJ = new double[3 * n];
		cJ = new double[3 * n];
