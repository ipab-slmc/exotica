...
SpaceBounds::SpaceBounds(double xlow, double xup, double ylow, double yup, double zlow, double zup) :
    x_low(xlow), x_upper(xup), y_low(ylow), y_upper(yup), z_low(zlow), z_upper(zup)
{
...
{
  return p.x >= x_low && p.x <= x_upper && p.y >= y_low && p.y <= y_upper && p.z >= z_low && p.z <= z_upper;
}

bool SpaceBounds::inBounds(const Eigen::Affine3d &p)
{
  //The translation part is at (12,13,14)
  return p.data()[12] >= x_low && p.data()[12] <= x_upper && p.data()[13] >= y_low && p.data()[13] <= y_upper
...
VolumeBounds::VolumeBounds(const SpaceBounds &bounds, double volume_resolution)
{
  ROS_WARN_STREAM(
...
void SparseBitSets::setFromBoolVector(const std::vector<bool> &bitset)
{
  ...
  if (!find_first)
  {
    std::pair<unsigned int, unsigned int> tmp_pair(tmp_first, bitset.size() - 1);
    tmp.push_back(tmp_pair);
  }
...
bool DRMSpace::createSpace(const SpaceBounds &bounds, double volume_resolution,
                           ...
                           const std::string & group_name)
{
  ...
  {
    ROS_ERROR_STREAM("Robot "<<model->getName()<<" does not have group "<<group_name);
    return false;
  }
...
bool DRMSpace::getVolumeIndex(const geometry_msgs::Point &p, unsigned int & index)
{
  ...
  index = x + y * volume_bounds_.bx + z * volume_bounds_.bx * volume_bounds_.by;
...
std::vector<std::pair<unsigned int, double> > DRMSpace::getNeighborIndices(unsigned int index, unsigned int depth)
{
  ...
  for (double x = -0.5 * resolution_ * depth; x <= 0.5 * resolution_; x += 0.5 * resolution_)
    for (double y = -0.5 * resolution_ * depth; y <= 0.5 * resolution_; y += 0.5 * resolution_)
      for (double z = -0.5 * resolution_ * depth; z <= 0.5 * resolution_; z += 0.5 * resolution_)
      {
        ...
        {
          std::pair<unsigned int, double> tmp(n_index, sqrt(getDistance(index, n_index)));
          ret.push_back(tmp);
        }
      }
  return ret;
...
{
  return sqrt(
      pow(volumes_[a].center.x - volumes_[b].center.x, 2) + pow(volumes_[a].center.y - volumes_[b].center.y, 2)
...
{
  return volumes_[z * volume_bounds_.bz * volume_bounds_.by + y * volume_bounds_.by + x];
}
...
void DRMSpace::addOccupSample(unsigned int volume_index, unsigned long int sample_index)
{
  boost::mutex::scoped_lock(volume_locks_[volume_index]);
  volumes_[volume_index].occup_samples.push_back(sample_index);
}

void DRMSpace::addReachSample(unsigned int volume_index, unsigned long int sample_index)
{
  boost::mutex::scoped_lock(volume_locks_[volume_index]);
...
void DRMSpace::setVolumeOccupied(unsigned int index, bool invalidate_samples)
{
  if (index < space_size_)
  {
    ...
    {
      for (unsigned long int i = 0; i < volumes_[index].occup_samples.size(); i++)
        samples_[volumes_[index].occup_samples[i]].invalidate();
      for (unsigned long int i = 0; i < volumes_[index].occup_edges.size(); i++)
...
std::vector<unsigned int> DRMSpace::getVolumeReachabilities()
{
  ...
  for (unsigned int i = 0; i < space_size_; i++)
  {
    ...
    for (unsigned long int j = 0; j < volumes_[i].reach_samples.size(); j++)
      if (samples_[volumes_[i].reach_samples[j]].isValid)
        tmp++;
    ret[i] = tmp;
  }
...
unsigned long int DRMSpace::CurrentlyReachability(unsigned int index, std::vector<unsigned long int> & valid_samples)
{
  valid_samples.clear();
...
void DRMSpace::buildConnectionGraph(double dmax)
{
  ...
  {
    std::pair<unsigned int, unsigned int> volumes(i * tmp, i == thread_size_ - 1 ? space_size_ : (i + 1) * tmp);
    th[i] = new boost::thread(boost::bind(&DRMSpace::buildGraphThreadFn, this, i, volumes, dmax));
  }
  for (unsigned int i = 0; i < thread_size_; ++i)
  {
    th[i]->join();
  ...
  for (unsigned int t = 0; t < thread_size_; ++t)
  {
    for (unsigned int te = 0; te < th_edges_[t].size(); te++)
    {
      ...
      for (unsigned int teo = 0; teo < th_edges_[t][te].second.size(); teo++)
        volumes_[th_edges_[t][te].second[teo]].occup_edges.push_back(edge_size);
      edge_size++;
    }
  }
...
void DRMSpace::buildGraphThreadFn(int thread_id, std::pair<unsigned int, unsigned int> &volumes, double dmax)
{
...
//      ROS_INFO_STREAM("Thread 3 ["<<volumes.first<<"-"<<volumes.second<<"]: "<<v);
    ...
    std::vector<std::pair<unsigned int, double> > neighbours = getNeighborIndices(v, 1);
    ...
    for (unsigned long int s = 0; s < volumes_[v].reach_samples.size(); s++)
    {
      ...
      for (unsigned int n = 0; n < neighbours.size(); n++)
      {
        ...
        for (unsigned long int sn = 0; sn < volumes_[neighbours[n].first].reach_samples.size(); sn++)
        {
          unsigned long int n_index = volumes_[neighbours[n].first].reach_samples[sn];
          //Check if the edge already exists
          if (newEdge(index, n_index))
          {
            double dist = (Eigen::Map<Eigen::VectorXf>(samples_[index].q, dimension_)
                - Eigen::Map<Eigen::VectorXf>(samples_[n_index].q, dimension_)).norm();    //.cwiseAbs().maxCoeff();
            ...
            if (dist < sqrt(dimension_) * dmax)
            {
              ...
              for (unsigned int vo = 0; vo < sample_occupation_[index].size(); vo++)
                edge_volume_occup[sample_occupation_[index][vo]] = true;
              for (unsigned int vo = 0; vo < sample_occupation_[n_index].size(); vo++)
                if (edge_volume_occup.find(sample_occupation_[n_index][vo]) == edge_volume_occup.end())
  ...
  ROS_INFO_STREAM("Thread "<<thread_id<<" finished with "<<th_edges_[thread_id].size()<<" edges, dmax="<<dmax);
}
