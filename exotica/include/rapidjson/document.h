...
#pragma pack (push, 4)
  ...
  class GenericValue
  {
    ...
    public:
      ...
      GenericValue(Type type)
      {
        static const unsigned defaultFlags[7] =
            { kNullFlag, kFalseFlag, kTrueFlag, kObjectFlag, kArrayFlag,
                kConstStringFlag, kNumberFlag | kIntFlag | kUintFlag
                    | kInt64Flag | kUint64Flag | kDoubleFlag };
      ...
      GenericValue(int i)
          : flags_(kNumberIntFlag)
      {
        data_.n.i64 = i;
        if (i >= 0) flags_ |= kUintFlag | kUint64Flag;
      }

      //! Constructor for unsigned value.
      GenericValue(unsigned u)
          : flags_(kNumberUintFlag)
      {
        data_.n.u64 = u;
        if (!(u & 0x80000000)) flags_ |= kIntFlag | kInt64Flag;
      }

      //! Constructor for int64_t value.
      GenericValue(int64_t i64)
          : flags_(kNumberInt64Flag)
      {
        data_.n.i64 = i64;
        if (i64 >= 0)
        {
          flags_ |= kNumberUint64Flag;
          if (!(i64 & 0xFFFFFFFF00000000LL)) flags_ |= kUintFlag;
          if (!(i64 & 0xFFFFFFFF80000000LL)) flags_ |= kIntFlag;
        }
        else if (i64 >= -2147483648LL) flags_ |= kIntFlag;
      }
      ...
      {
        data_.n.u64 = u64;
        if (!(u64 & 0x8000000000000000ULL)) flags_ |= kInt64Flag;
        if (!(u64 & 0xFFFFFFFF00000000ULL)) flags_ |= kUintFlag;
        if (!(u64 & 0xFFFFFFFF80000000ULL)) flags_ |= kIntFlag;
      }

      //! Constructor for double value.
      GenericValue(double d)
          : flags_(kNumberDoubleFlag)
      {
        data_.n.d = d;
      }
        ...
        if (o.size >= o.capacity)
        {
          ...
          else
          {
            ...
            o.members = (Member*) allocator.Realloc(o.members,
                oldCapacity * sizeof(Member), o.capacity * sizeof(Member));
          }
        }
      ...
      GenericValue& AddMember(const Ch* name, Allocator& nameAllocator,
          GenericValue& value, Allocator& allocator)
      {
      ...
      GenericValue& PushBack(GenericValue& value, Allocator& allocator)
      {
        ...
        if (data_.a.size >= data_.a.capacity)
          Reserve(
              data_.a.capacity == 0 ?
                  kDefaultArrayCapacity : data_.a.capacity * 2, allocator);
      ...
      const GenericValue& Accept(Handler& handler) const
      {
        ...
        case kObjectType:
          handler.StartObject();
          for (Member* m = data_.o.members; m != data_.o.members + data_.o.size;
              ++m)
          {
    ...
    private:
      ...
      Member* FindMember(const Ch* name)
      {
        ...
        Object& o = data_.o;
        for (Member* member = o.members;
            member != data_.o.members + data_.o.size; ++member)
          if (length == member->name.data_.s.length
              && memcmp(member->name.data_.s.str, name, length * sizeof(Ch))
                  == 0) return member;
      ...
      {
        flags_ = kArrayFlag;
        data_.a.elements = (GenericValue*) alloctaor.Malloc(
...
//! A document for parsing JSON text as DOM.
  ...
  class GenericDocument: public GenericValue<Encoding, Allocator>
  {
    public:
       ...
       */
      GenericDocument(Allocator* allocator = 0, size_t stackCapacity =
    ...
    private:
      ...
      {
        typename ValueType::Member* members = stack_.template Pop<
            typename ValueType::Member>(memberCount);
