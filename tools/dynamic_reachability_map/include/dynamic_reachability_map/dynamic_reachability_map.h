...
namespace dynamic_reachability_map
{
  ...
  class DynamicReachabilityMap
  {
    public:
      ...
      bool AStarPathPlanning(int start, const Eigen::VectorXd &start_q,
          int goal, std::vector<int> &path);
      ...
      void pathSimplifier(const Eigen::MatrixXd & path,
  ...
  class MultiThreadsSpaceOccupationLoader
  {
    public:
      MultiThreadsSpaceOccupationLoader(const std::string & path,
          int thread_cnt, int space_size, int sample_size)
