                               ...
                               const robot_model::RobotModelConstPtr &model)
{
  ...
  tinyxml2::XMLHandle drm_handle(xml_file.FirstChildElement("DynamicReachabilityMap"));
  ...
  {
    ROS_ERROR_STREAM("Robot model has name "<<model->getName()<<", but DRM is created with "<<robot_name);
    return false;
  }
  ...
  tinyxml2::XMLHandle group_handle = drm_handle.FirstChildElement("PlanningGroup");
  ...
  {
    ROS_ERROR_STREAM("Robot "<<model->getName()<<" does not have end-effector link "<<eff);
    return false;
  }
  ...
  if (!tmp_handle.ToElement())
  {
    ROS_ERROR("DynamicReachabilityMap does not have OccupationSavingThreads tag");
    return false;
  }
  ...
  for (int i = 0; i < sample_size; i++)
  {
    ...
    samples_file.read((char *)&space->samples_[i].eff_index, sizeof(unsigned int));
    samples_file.read((char *)&space->samples_[i].effpose.position.x, sizeof(float));
    samples_file.read((char *)&space->samples_[i].effpose.position.y, sizeof(float));
    samples_file.read((char *)&space->samples_[i].effpose.position.z, sizeof(float));
    samples_file.read((char *)&space->samples_[i].effpose.orientation.x, sizeof(float));
    samples_file.read((char *)&space->samples_[i].effpose.orientation.y, sizeof(float));
    samples_file.read((char *)&space->samples_[i].effpose.orientation.z, sizeof(float));
    samples_file.read((char *)&space->samples_[i].effpose.orientation.w, sizeof(float));
...
//      space->samples_[i].q[j] = std::stod(sample[j + 8]);
  ...
  for (int i = 0; i < space->space_size_; i++)
  {
    ...
    for (int j = 0; j < reaches.size(); j++)
      space->volumes_[i].reach_samples.push_back((unsigned long int)std::stoi(reaches[j]));
  }

  //Loading occupation samples
  ROS_INFO("Loading occupation samples");
  boost::shared_ptr<MultiThreadsSpaceOccupationLoader> mtl;
  mtl.reset(new MultiThreadsSpaceOccupationLoader(path + "/space_occupation", space));
  mtl->loadSpaceOccupation();
  return true;
}

DRMSpaceLoader::MultiThreadsSpaceOccupationLoader::MultiThreadsSpaceOccupationLoader(const std::string & path,
                                                                                     DRMSpace_ptr &space) :
...
void DRMSpaceLoader::MultiThreadsSpaceOccupationLoader::loadSpaceOccupation()
{
  ...
  for (unsigned int i = 0; i < space_->thread_size_; ++i)
    th[i] = new boost::thread(boost::bind(&MultiThreadsSpaceOccupationLoader::load, this, i));
  for (unsigned int i = 0; i < space_->thread_size_; ++i)
  {
    th[i]->join();
    delete th[i];
...
void DRMSpaceLoader::MultiThreadsSpaceOccupationLoader::load(int id)
{
  ...
  std::ifstream space_occupied_file(path_ + "/space_occupation_" + std::to_string(id) + ".bin", std::ios_base::binary);
  ...
  while (!space_occupied_file.eof())
  {
    ...
    else
    {
      ...
      space_occupied_file.read((char *)&space_->volumes_[index].occup_samples[0],
