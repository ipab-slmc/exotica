...
bool DRM::initialise(const std::string &drms_path, const robot_model::RobotModelConstPtr &model)
{
  ...
  W_ = Eigen::MatrixXd::Ones(4 + space_->getDimension(), 4 + space_->getDimension());
  for (int i = 0; i < 4; i++)
    W_(i, i) = 1e2;
  return true;
}
...
dynamic_reachability_map::DRMResult DRM::getIKSolution(const dynamic_reachability_map::DRMGoalConstPtr &goal)
{
  ...
  else
  {
    ...
    std::vector<std::pair<unsigned int, double> > neighbours = space_->getNeighborIndices(index, 1);
    for (unsigned int i = 0; i < neighbours.size(); i++)
    {
      if (space_->CurrentlyReachability(neighbours[i].first, candidate_samples) > 0
  ...
  ROS_WARN_STREAM("IK request time: "<<ros::Duration(ros::Time::now()-start_time).toSec()<<"sec");
  return result;
}

bool DRM::solve(const dynamic_reachability_map::DRMGoalConstPtr &goal, unsigned long int goal_sample_index,
                std::vector<unsigned long int> &path)
{
  ...
  for (int i = 0; i < 7; i++)
    q0.data[i] = 0;
  if (!getClosestSample(q0, start_sample_index))
    return false;
  ROS_WARN_STREAM(
...
bool DRM::searchAStar(unsigned long int start_sample_index, unsigned long int goal_sample_index,
                      std::vector<unsigned long int> &path)
{
  ROS_INFO_STREAM("AStar from "<<start_sample_index<<" to "<<goal_sample_index);
  ...
  f_score[start_sample_index] = g_score[start_sample_index] + heuristicCost(start_sample_index, goal_sample_index);
  ...
  while (open.size() > 0)
  {
    ...
    for (std::map<unsigned long int, bool>::iterator it = open.begin(); it != open.end(); it++)
    {
    ...
    space_->getVolumeIndex(space_->getSample(current_index).effpose.position, current_volume);
    ...
    std::vector<unsigned long int> edges = space_->samples_[current_index].edges;
...
//      ROS_INFO_STREAM("Edge "<<edges[i]<<" a="<<space_->edges_[edges[i]].a<<",b="<<space_->edges_[edges[i]].b);
      unsigned long int neighbour_index =
          current_index == space_->edges_[edges[i]].a ? space_->edges_[edges[i]].b : space_->edges_[edges[i]].a;
//      ROS_INFO_STREAM("Sample "<<current_index<<" process edge "<<i<<"/"<<edges.size()<<" Neighbour "<<neighbour_index);
      ...
      {
        double tentative_g = g_score[current_index] + 0.1 * space_->edges_[edges[i]].length;

        if (open.find(neighbour_index) == open.end() || tentative_g < g_score.at(neighbour_index))
        {
//          ROS_INFO_STREAM("Process "<<neighbour_index);
          came_from[neighbour_index] = current_index;
          g_score[neighbour_index] = tentative_g;
          f_score[neighbour_index] = tentative_g + heuristicCost(neighbour_index, goal_sample_index);
          if (open.find(neighbour_index) == open.end())
          {

            unsigned int neighbour_volume;
            space_->getVolumeIndex(space_->getSample(neighbour_index).effpose.position, neighbour_volume);
...
bool DRM::getClosestSample(const dynamic_reachability_map::DRMGoalConstPtr &goal,
...
bool DRM::getClosestSample(const exotica::Vector &q, unsigned long int &sample_index)
{
  Eigen::VectorXf q_eigen(q.data.size());
  for (int i = 0; i < space_->dimension_; i++)
  {
    space_->ps_->getCurrentStateNonConst().setVariablePosition(space_->var_index_[i], q.data[i]);
    q_eigen(i) = q.data[i];
  }
  space_->ps_->getCurrentStateNonConst().update(true);

  Eigen::Affine3d effpose = space_->ps_->getCurrentStateNonConst().getGlobalLinkTransform(space_->eff_);
  ...
  double dist = INFINITY;
  for (unsigned long int i = 0; i < space_->volumes_[volume_index].reach_samples.size(); i++)
  {
    double tmp_dist = (Eigen::Map<Eigen::VectorXf>(space_->samples_[space_->volumes_[volume_index].reach_samples[i]].q,
