...
bool DynamicReachabilityMap::createSpace(const Eigen::Vector2d & bx,
		const Eigen::Vector2d & by, const Eigen::Vector2d & bz,
		double cellsize) {
	bounds_.resize(3);
	bounds_[0] = bx;
	bounds_[1] = by;
...
bool DynamicReachabilityMap::createRobotSampler(
	...
	if (!model->hasLinkModel(eff)) {
		ROS_ERROR_STREAM(
				"Robot "<<model->getName()<<" does not have link "<<eff);
		return false;
	}
...
bool DynamicReachabilityMap::indexingSpace(int sample_cnt,
	...
	for (int i = 0; i < sample_size_; ++i) {
		if (thread_id_ == 0 && i % (sample_size_ / 1000) == 0) {
			std::cout << "Sampling process " << i << " / " << sample_size_
					<< "(" << (double) i / (double) sample_size_ * 100.0
					<< "%).\r";
			std::cout.flush();
		}
		bool valid = false;
		...
		if (result.collision) {
			...
			for (auto &it : result.contacts) {
				if (cell_name_map.find(it.first.first) == cell_name_map.end()
						&& cell_name_map.find(it.first.second)
								== cell_name_map.end()) {
					sample_ok = false;
					break;
				}
			...
			if (sample_ok) {
				...
				for (auto &it : result.contacts) {
					int index = 0;
					if (cell_name_map.find(it.first.first)
							!= cell_name_map.end()) {
						index = cell_name_map.at(it.first.first);
					} else if (cell_name_map.find(it.first.second)
							!= cell_name_map.end()) {
...
bool DynamicReachabilityMap::solve(const Eigen::VectorXd & q0,
	...
	if (density_[goal_index] <= 0) {
		ROS_ERROR_STREAM(
				"Invalid goal state, no sample reaches this goal cell");
		return false;
	}
	...
	for (std::map<int, double>::iterator it = neighbors.begin();
		...
		for (int j = 0; j < cell_sample_cnt; j++) {
			if (samples_[tcps_[it->first].samples[j]].valid) {
				double tmp_dist = (samples_[tcps_[it->first].samples[j]].q
		...
		for (int i = 0; i < path.size(); i++) {
			...
			for (int j = 0; j < cell_sample_cnt; j++) {
				if (samples_[tcps_[path[i]].samples[j]].valid) {
					double tmp_dist = (samples_[tcps_[path[i]].samples[j]].q
							- current).cwiseAbs().maxCoeff();
					if (tmp_dist < dist) {
						dist = tmp_dist;
			...
			if (sample_index == -1) {
				ROS_ERROR_STREAM(
						"No close sample is found at tcp path "<<path[i]);
				return false;
			}
...
///	Lets do A* !
	...
	while (open.size() > 0) {
		...
		for (std::map<int, double>::iterator it = open.begin();
				it != open.end(); it++) {
			if (it->second < min)
				current = it;
		}
...
bool DynamicReachabilityMap::saveSpace(const std::string & path,
	...
	for (int i = 0; i < samples_.size(); i++) {
		samples_file << samples_[i].tcp.position.x << " "
				<< samples_[i].tcp.position.y << " "
				<< samples_[i].tcp.position.z << " "
	...
	for (int t = 0; t < multiThreads; t++) {
		...
		space_occupied_file.open(
				folderpath + "/space_occupation/space_occupation_"
						+ std::to_string(t) + ".txt");
...
bool DynamicReachabilityMap::createCellURDF(const std::string & path) {
	...
	for (int i = 0; i < space_size_; i++) {
		urdf << "<joint name=\"joint_" << i << "\" type=\"fixed\">"
				<< std::endl;
