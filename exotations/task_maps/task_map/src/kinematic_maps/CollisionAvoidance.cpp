...
namespace exotica
{
  ...
  EReturn CollisionAvoidance::initDerived(tinyxml2::XMLHandle & handle)
  {
    ...
    {
      close_pub_ = server_->advertise<visualization_msgs::Marker>(
          object_name_ + "/close_marker", 100);
      robot_centre_pub_ = server_->advertise<visualization_msgs::Marker>(
          object_name_ + "/robot_centre_marker", 100);
      world_centre_pub_ = server_->advertise<visualization_msgs::Marker>(
          object_name_ + "/world_centre_marker", 100);
      ...
      robot_centre_.header.frame_id = close_.header.frame_id = "/"
          + kin_sol_.getRootName();
      world_centre_.header.frame_id = close_.header.frame_id = "/"
          + kin_sol_.getRootName();
      HIGHLIGHT_NAMED(object_name_,
          "Collision avoidance taskmap running in debug mode, collision info will be published to marker msgs.");
    }
    return SUCCESS;
  }
    ...
    for (int i = 0; i < M; i++)
    {
      ...
      else
      {
        ...
        if (use_vel_->data && !first)
        {
          ...
          {
            discounts(i) = ct;
            HIGHLIGHT_NAMED(effs_[i],
                "CosTheta "<<ct<<" Discount "<<discounts(i));
          }
        }
      ...
      if (kin_sol_.getEffSize() > 0)
      {
        ...
        for (int i = 0; i < M; i++)
        {
          if (dists[i] <= 0)
          {
            ...
            JAC +=
                ((2.0 * discounts(i) * costs[i]) / safe_range_->data)
                    * (norms[i].transpose()
                        * effJac.block(3 * i, 0, 3, JAC.cols()));
        }
      }
    }
