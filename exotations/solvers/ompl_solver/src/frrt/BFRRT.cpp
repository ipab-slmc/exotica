...
namespace ompl
{
  namespace geometric
  {
    ...
    {
      maxDistance_ = 0.0;
      Planner::declareParam<double>("range", this, &BFRRT::setRange,
          &BFRRT::getRange, "0.:1.:10000.");
    ...
    void BFRRT::setup()
    {
      ...
      if (!tStart_)
        tStart_.reset(
            tools::SelfConfig::getDefaultNearestNeighbors<Motion*>(
                si_->getStateSpace()));
      if (!tGoal_)
        tGoal_.reset(
            tools::SelfConfig::getDefaultNearestNeighbors<Motion*>(
      ...
      tStart_->setDistanceFunction(
          boost::bind(&BFRRT::distanceFunction, this, _1, _2));
    ...
    BFRRT::GrowState BFRRT::growTree(TreeData &tree, TreeGrowingInfo &tgi,
        Motion *rmotion)
    {
