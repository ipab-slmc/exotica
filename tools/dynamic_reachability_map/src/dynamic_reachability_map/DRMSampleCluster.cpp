...
DRMClusterParam::DRMClusterParam() :
    min_num_clusters(1), small_cluster_cutoff(0.02), max_joint_step(M_PI / 8), tolerance(1e-5), algorithm("MeanShift")
{
...
{
  algorithms_["MeanShift"] = boost::bind(&DRMSampleCluster::meanShiftClustering, this, _1);
}
...
bool DRMSampleCluster::startClustering(DRMSpace_ptr &space, DRMClusterParam &param)
{
...
void DRMSampleCluster::processOccupation()
{
  ...
  for (unsigned int i = 0; i < space_->getSpaceSize(); i++)
  {
    ...
    volume_obj.header.frame_id = space_->ps_->getRobotModel()->getRootLinkName();
    volume_obj.id = std::to_string(i);
    volume_map_[volume_obj.id] = i;
    shape_msgs::SolidPrimitive cell;
    cell.type = cell.BOX;
    cell.dimensions.resize(3);
    cell.dimensions[0] = cell.dimensions[1] = cell.dimensions[2] = space_->getResolution();
  ...
  for (unsigned int i = 0; i < space_->space_size_; i++)
  {
    ...
    {
      ROS_INFO_STREAM("Processing cluster occupation "<<tmp_sample<<"/"<<clusters_cnt_);
      for (int n = 0; n < var_index_.size(); n++)
        space_->ps_->getCurrentStateNonConst().setVariablePosition(var_index_[n], clusters_[i][j].center(n));
      space_->ps_->getCurrentStateNonConst().update(true);
      tf::poseEigenToMsg(space_->ps_->getCurrentStateNonConst().getGlobalLinkTransform(space_->eff_),
                         space_->getSampleNonConst(tmp_sample).effpose);

      space_->qEigen2Array(clusters_[i][j].center, space_->getSampleNonConst(tmp_sample).q);
      unsigned int tmp_check = 0;
      space_->getVolumeIndex(space_->getSampleNonConst(tmp_sample).effpose.position, tmp_check);
...
{
  ROS_WARN_STREAM_NAMED("DRM MeanShift", "Minimum number of clusters per volume "<<param.min_num_clusters);
  ROS_WARN_STREAM_NAMED("DRM MeanShift", "Small clusters cutoff factor "<<param.small_cluster_cutoff);
  ROS_WARN_STREAM_NAMED("DRM MeanShift", "Maximum joint step "<<param.max_joint_step);
  ...
  {
    std::pair<unsigned int, unsigned int> volumes(i * tmp, i == thread_size - 1 ? space_->space_size_ : (i + 1) * tmp);
    th[i] = new boost::thread(boost::bind(&DRMSampleCluster::meanShiftThreadFn, this, i, volumes, param));
  }
  for (unsigned int i = 0; i < thread_size; ++i)
  {
    th[i]->join();
  ...
  ROS_WARN_STREAM("Clustering finished !!!!!! "<<space_->sample_size_<<" are clustered into "<<clusters_cnt_);
}

void DRMSampleCluster::meanShiftThreadFn(int id, std::pair<unsigned int, unsigned int> &volumes, DRMClusterParam &param)
{
...
void DRMSampleCluster::meanShiftClusterVolume(int thread_id, unsigned int index, DRMClusterParam &param)
{
  ...
  if (sample_size <= param.min_num_clusters)
  {
    ...
    {
      Cluster tmp_cluster;
      space_->qArray2Eigen(space_->samples_[space_->volumes_[index].reach_samples[i]].q, tmp_cluster.center);
  ...
  else
  {
    ...
    while (cnt > 0)
    {
      ...
      while (iteration < sample_size)
      {
        ...
        {
          double max_dist =
              (old_mean - Eigen::Map<Eigen::VectorXf>(space_->samples_[samples[i]].q, space_->dimension_)).norm();//cwiseAbs().maxCoeff();
          if (max_dist < sqrt(space_->getDimension())*param.max_joint_step)
          {
            this_votes(i) = this_votes(i) + 1;
            my_members.push_back(samples[i]);
            open[i] = false;
            my_mean += Eigen::Map<Eigen::VectorXf>(space_->samples_[samples[i]].q, space_->dimension_);
          }
        }
        ...
        if ((my_mean - old_mean).norm() < param.tolerance)
        {
          ...
          {
            if ((my_mean - it.second.center).norm() < sqrt(space_->dimension_) * param.max_joint_step / 2)
            {
              new_cluster = false;
    ...
    for (int i = 0; i < votes_T.rows(); i++)
    {
      ...
      clusters[max_index].samples.push_back(space_->volumes_[index].reach_samples[i]);
      sample_clustered_[space_->volumes_[index].reach_samples[i]] = true;
    }
    ...
    for (unsigned int i = 0; i < clusters.size(); i++)
    {
      if (clusters.size() <= param.min_num_clusters
          || clusters[i].samples.size() > param.small_cluster_cutoff * sample_size)
      {
        for (int n = 0; n < var_index_.size(); n++)
          space_->ps_->getCurrentStateNonConst().setVariablePosition(var_index_[n], clusters[i].center(n));
        space_->ps_->getCurrentStateNonConst().update(true);
        geometry_msgs::Pose tmppose;
        tf::poseEigenToMsg(space_->ps_->getCurrentStateNonConst().getGlobalLinkTransform(space_->eff_), tmppose);
        unsigned int tmp_check = 0;
