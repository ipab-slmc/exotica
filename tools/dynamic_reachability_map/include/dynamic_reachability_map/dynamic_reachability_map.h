...
public:
   ...
   */
  bool createSpace(const Eigen::Vector2d & bx, const Eigen::Vector2d & by, const Eigen::Vector2d & bz, double cellsize);
   ...
   */
  bool createRobotSampler(const robot_model::RobotModelConstPtr &model, const std::string & eff,
                          const std::string & group_name);
  bool createSpacePlanningScene(moveit_msgs::PlanningSceneWorld &world, std::map<std::string, int> &cell_name_map);
  bool setSpacePlanningScene(const moveit_msgs::PlanningSceneWorld world);
  bool indexingSpace(int sample_cnt, std::map<std::string, int> cell_name_map);
  ...
  bool solve(const Eigen::VectorXd & q0, const geometry_msgs::Pose & goal, Eigen::MatrixXd & solution,
  ...
  bool AStarPathPlanning(int start, const Eigen::VectorXd &start_q, int goal, std::vector<int> &path);
  ...
  bool checkPathDetail(const std::vector<int> &sample_path, std::pair<int, int> &disconnect);
  ...
  bool meanShiftClustering(double h);
  bool meanShiftCell(int index, double h, std::vector<Cluster> &clusters);
  bool createCellURDF(const std::string & path);
  void pathSimplifier(const Eigen::MatrixXd & path, Eigen::MatrixXd &simple);
  bool isStateValid(const ompl::base::State *state);
//		private:

  std::vector<std::vector<int>> sample_occupation_;
...
{
public:
  MultiThreadsIndexer(DynamicReachabilityMap &is, int thread_cnt, int sample_cnt) :
      thread_cnt_(thread_cnt), sample_size_(sample_cnt)
  {
    ...
    {
      drm_[i].createSpace(is.bounds_[0], is.bounds_[1], is.bounds_[2], is.cell_size_);
      drm_[i].createRobotSampler(is.ps_->getRobotModel(), is.eff_, is.group_->getName());
      drm_[i].thread_id_ = i;
    }
  }
  void indexingSpace()
  {
    ...
    for (unsigned int i = 0; i < thread_cnt_; ++i)
      th[i] = new boost::thread(boost::bind(&MultiThreadsIndexer::indexing, this, i, sample_size_ / thread_cnt_));
    for (unsigned int i = 0; i < thread_cnt_; ++i)
    {
      th[i]->join();
      delete th[i];
  ...
  bool combineSamples(DynamicReachabilityMap &is)
  {
    ...
    for (int i = 0; i < thread_cnt_; i++)
    {
      ...
      is.samples_.insert(is.samples_.end(), drm_[i].samples_.begin(), drm_[i].samples_.end());
      for (int j = 0; j < is.space_size_; j++)
      {
        for (int l = 0; l < drm_[i].space_[j].occupiedSmaples.size(); l++)
        {
          int tmp = drm_[i].space_[j].occupiedSmaples[l] + current_start;
...
{
public:
  MultiThreadsSpaceOccupationLoader(const std::string & path, int thread_cnt, int space_size, int sample_size) :
      path_(path), thread_cnt_(thread_cnt), space_size_(space_size), sample_size_(sample_size)
  {
  ...
  void loadSpaceOccupation()
  {
    ...
    for (unsigned int i = 0; i < thread_cnt_; ++i)
      th[i] = new boost::thread(
          boost::bind(&MultiThreadsSpaceOccupationLoader::load, this, i, space_size_ / thread_cnt_));
    for (unsigned int i = 0; i < thread_cnt_; ++i)
    {
      th[i]->join();
      delete th[i];
