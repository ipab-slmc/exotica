    ...
    EReturn IMesh::computeLaplace(int t)
    {
        ...
        for (j = 0; j < N; j++)
        {
            for (l = j + 1; l < N; l++)
            {
                if (!(j >= N && l >= N))
                {
                    dist(j, l) = dist(l, j) = sqrt( (EFFPHI.segment(j,3) - EFFPHI.segment(l,3)).dot(( EFFPHI.segment(j,3) - EFFPHI.segment(l,3) )) );
                }
            }
        }
                            ...
                            if (dist(j, l) > 0 && wsum(j) > 0 && weights_(j, l) > 0)
                            {
                                ...
                                if (j < M)
                                {
                                    if (l < M)
                                        ...
                                        _distance = EFFJAC.block(3 * j, i, 3, 1) - EFFJAC.block(3 * l, i, 3, 1);
                                    else
                                ...
                                for (k = 0; k < M; k++)
                                {
                                    if (j != k && dist(j, k) > 0 && weights_(j, k) > 0)
                                    {
                                        ...
                                        {
                                            if (k < M)
                                                _distance = -EFFJAC.block(3 * k, i, 3, 1);
                                        }
